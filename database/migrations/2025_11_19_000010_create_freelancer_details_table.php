...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('freelancer_details', function (Blueprint $table) {
            $table->integer('freelancer_detail_id')->autoIncrement();
            ...
            $table->unique('user_id', 'freelancer_details_user_id_unique');
            $table->index('user_id', 'idx_freelancer_user_id');

            $table->foreign('user_id', 'fk_freelancer_details_user')
                ->references('user_id')
                ->on('users')
                ->onDelete('cascade')
                ->onUpdate('cascade');
